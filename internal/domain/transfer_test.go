...
// поэтому веб и движок показывали разные расходы — расхождение ровно на одну
// запись. Два счёта одних денег: чей прав, по числам не понять.
func TestIsInternalTransfer(t *testing.T) {
	for _, c := range []struct {
		...
		{"расход-перевод себе", expenseTx(t, "Прочее", "Перевод себе", ""), true},
		{"расход-перевод себе, регистр и пробелы", expenseTx(t, " прочее ", "перевод  себе", ""), true},
		// ⚠️ Замер на живых данных: единственная запись с подкатегорией
		// «Переводы» оказалась возвратом переплаты ЗАКАЗЧИКУ. Деньги ушли
		// по-настоящему, а движок исключал их из расходов — потому что слово
		// «Переводы» не отвечает на вопрос «кому».
		{"перевод кому-то — настоящий расход", expenseTx(t, "Прочее", "Переводы", ""), false},
