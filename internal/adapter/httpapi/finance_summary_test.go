...
func TestFinanceSummaryServesEveryCut(t *testing.T) {
	...
	//
	// Множество берётся ИЗ САМОГО DTO, а не из списка, вписанного сюда: прежняя
	// редакция перечисляла 12 полей из 15, и `byAccount` с обоими полями про
	// исключённые переводы могли пропасть из ответа молча (issue #229).
	keys := httpapi.FinanceSummaryKeys()

	// ⚠️ Якорь на число. Без него проверка замыкается сама на себя: множество
	// берётся из json-тегов, и поле, помеченное `json:"-"` или удалённое из
	// DTO, просто исчезает из ОБЕИХ сторон сравнения — подсадка проходит молча,
	// я это замерил. Число правится руками и только осознанно: разрез, убранный
	// из сводки, обязан быть замечен человеком, потому что во фронте он
	// исчезает без следа.
	const cuts = 15
	if len(keys) != cuts {
		t.Errorf("разрезов в сводке %d, ждали %d — если разрез убран намеренно, "+
			"поправьте число здесь и проверьте, что фронт его больше не читает", len(keys), cuts)
	}
