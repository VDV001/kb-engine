...
// Примеры действий из лида доезжают до страницы.
//
// Обе страницы-предшественницы объясняли фразу «не список модулей, а действия
// целиком» примерами — «владелец записывает трату», «сбор кандидатов в 10:03».
// При переносе на вкладку фраза осталась, примеры пропали, и она стала
// утверждением ни о чём. Место примеров в карте, а не в вёрстке: у каждой карты
// они свои, а вкладка одна на обе.
func TestLoad_leadExamples(t *testing.T) {
	m := loadOne(t, strings.Replace(engineMap,
		`"project": "test-engine",`,
		`"project": "test-engine", "examples": ["владелец записывает трату", "страница сама говорит, что отстала"],`,
		1))

	if len(m.Examples) != 2 {
		t.Fatalf("Examples = %+v, ожидалось два примера", m.Examples)
	}
	if m.Examples[0] != "владелец записывает трату" {
		t.Errorf("Examples[0] = %q", m.Examples[0])
	}
}

// Карта без примеров оставляет их пустыми, а не выдумывает.
//
// Пустой список для страницы — законное состояние: она покажет лид без
// примеров. Выдуманный пример был бы хуже пропавшего, потому что выглядел бы
// как факт о проекте.
func TestLoad_leadExamplesAbsentStayEmpty(t *testing.T) {
	m := loadOne(t, engineMap)
