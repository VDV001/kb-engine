...
//
// specials — тоже материал Хабра со своим номером: на живом каталоге такая
// запись ровно одна, и найдена она не чтением кода, а прогоном миграции, после
// которого из 1266 адресов остался единственный неразобранный.
var habrArticleURL = regexp.MustCompile(`^https?://(?:[a-z0-9-]+\.)*habr\.com/\S*?(?:articles|post|blog|specials)/(\d+)`)
