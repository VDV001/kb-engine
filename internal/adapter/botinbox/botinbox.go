...
func MapArticle(a Article, now time.Time) (domain.EntryParams, error) {
	...
	// выдуманного.
	//
	// Прежняя редакция обосновывала это тем, что «по полю потом ищут дубли», и
	// обоснование было неверным. Дубли ищут по адресу и нормализованному
	// заголовку (audit.Duplicates), приём — по адресу (ingest.Plan). Номер
	// читают сверка задач с каталогом (taskaudit), предикат «это habr» и
	// витрина — там выдуманное значение соврало бы ровно так же, поэтому вывод
	// остаётся тем же, а причина у него теперь настоящая.
	//
	// Заводить дедуп по номеру живые данные не просят: записей с номером 1299,
	// повторов номера четыре, и у всех четырёх адрес один — их уже ловит
	// существующее правило (замер 12.08).
	var habrID *int
