...
// SortByRecency упорядочивает записи так, как их читает человек: сначала свежая
// дата, внутри дня — та, что записана позже. Момент записи спрашивается у
// домена, поэтому терминал и веб отвечают одинаково на вопрос «что записано
// последним».
//
// Строка с неизвестным моментом (её вписали в книгу мимо движка) считается
// самой ранней в своём дне — то же решение, что на витрине. Раньше такая строка
// вставала НАВЕРХ списка правки: порядок брался сравнением сырого id, а
// позиционный «expense-rN» больше любого ULID по алфавиту.
//
// Отдельно от Sort намеренно: Sort задаёт порядок ФАЙЛА, его читают люди и
// диффает git, поэтому там ties разрешает сам id — воспроизводимость важнее
// человеческого порядка. Здесь наоборот.
func SortByRecency(recs []Record) {
	slices.SortStableFunc(recs, func(a, b Record) int {
		if c := b.tx.Date().Compare(a.tx.Date()); c != 0 {
			return c
		}
		atA, okA := domain.RecordedAt(a.tx.ID())
		atB, okB := domain.RecordedAt(b.tx.ID())
		switch {
		case okA && okB:
			return atB.Compare(atA)
		case okA:
			return -1 // известный момент выше неизвестного
		case okB:
			return 1
		default:
			return 0 // оба неизвестны — остаётся порядок файла
		}
	})
}
