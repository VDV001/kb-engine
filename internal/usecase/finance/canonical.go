...
// Canonical приводит значения записи к написанию, которое в базе уже есть, и
// возвращает список подстановок.
//
...
func Canonical(existing []Record, p AddParams) (AddParams, []Correction) {
	return CanonicalWith(existing, Vocabulary{}, p)
}

// CanonicalWith делает то же, но словарь решает раньше частоты.
//
// Порядок именно такой, и он проверен живым случаем: в леджере «Пятерочка»
