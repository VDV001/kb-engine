...
type Index struct {
	Model string
	Dims  int
	// Built — когда индекс снят. Само по себе поле ничего не решает, но без
	// него сообщение о пробелах говорит, СКОЛЬКО записей слой не видел, и не
	// говорит, с каких пор.
	Built   string
...
//
// Нужна, чтобы поиск мог назвать, чего он НЕ проверял: индекс снят в один
// момент, каталог живёт дальше, и молчание об этом превращает «слой этой
// записи не видел» в неотличимое «в базе такого нет» (#254).
//
// Обратная сторона намеренно не считается недостачей: индекс шире каталога —
// это хвост от удалённых записей, и ругаться на уже почищенное значило бы
// заводить находку там, где работа сделана.
func (ix Index) Uncovered(ids []int) []int {
	var out []int
	for _, id := range ids {
