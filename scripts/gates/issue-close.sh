...
# strip_quoted убирает то, что ЦИТИРУЕТСЯ, а не употребляется: блоки кода,
# строки цитат и куски в обратных кавычках или «ёлочках».
#
# Без этого гейт падал на собственном PR, где ошибка приводится как пример.
# Проверка, ругающаяся на правду, будет отключена людьми — и тогда защиты не
# станет вовсе; это тот же класс, что ложные срабатывания на текстах про саму
# безопасность.
strip_quoted() {
  awk '
...
self_test() {
  local dir; dir=$(mktemp -d); local rc=0; local n=0
  mkdir -p "$dir/iss"
  ...
  # Употребление против упоминания: гейт обязан молчать там, где ошибку
  # ЦИТИРУЮТ. Иначе он падает на разборе собственной находки, а детектор,
  # ругающийся на правду, отключают.
  probe "цитата в ёлочках — не употребление"       'в описании стояло «Закрывает #5», это ошибка'  0
  probe "цитата в обратных кавычках"               'слово `Закрывает #5` не работает'              0
  probe "строка-цитата markdown"                   '> Закрывает #5'                                0
  probe "блок кода"                                '```
Закрывает #5
```'                                                                                                0
  probe "цитата рядом с настоящим употреблением"   'пример «Закрывает #5», а тут по-настоящему: Чинит #5'  1

  rm -rf "$dir"
  # Число проб СЧИТАЕТСЯ, а не пишется константой: захардкоженный счёт
  # разошёлся бы с таблицей на первой же новой пробе и врал бы зелёным.
  [[ "$rc" -eq 0 ]] && echo "самопроверка: $n из $n" || echo "самопроверка ПРОВАЛЕНА (проб: $n)"
  return "$rc"
}
