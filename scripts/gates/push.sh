...
# Вынесен в отдельный скрипт, потому что у него есть --self-test на подсадках,
# а push.sh целиком локально не прогонишь: он тянет за собой тесты, гонку и
# покрытие. Гейт, который нельзя проверить отрицательным контролем, проверяют
# только в бою — то есть уже после отправки.
#
# Что он смотрит и чего не видит — сказано в самом скрипте, а не здесь: место
# для этого одно, иначе описание и поведение разойдутся (так и вышло в #331).
if [ "${DATA_SKIP:-0}" != "1" ]; then
  if ! ./scripts/gates/owner-data.sh; then
    exit 1
  fi
fi
