...
PY
    ...
    #   • ✅ ЗАКРЫТО 27.08: счёт без единой транзакции в ledger не попадает вовсе —
    #     а именно так устроен долговой счёт (выдача меняет баланс, а не журнал), —
    #     и раньше самое чувствительное имя этот источник пропускал. Теперь имена
    #     дочитываются командой `kbengine fin accounts` с листа «Счета». Цена дыры
    #     замерена: такое имя стояло в публичном репозитории 17 раз, гейт молчал.
    #     ⚠️ Книги на CI нет — там список остаётся неполным, и это говорится вслух
    #     отдельной строкой, а не выглядит как «проверено и чисто».
...
PY
    ...
    # в журнале нет по конструкции. Спрашиваем ДВИЖОК, а не открываем книгу своим
    # парсером — он единственный писатель, и он же читает лист без потери строк
    # (режим read_only у openpyxl их терял, замер 04.08).
    book="${KB_BOOK:-$HOME/claude-cowork/finances/Учёт_финансов.xlsx}"
    if [ ! -f "$book" ] || ! command -v kbengine >/dev/null 2>&1; then
      ...
      # а `2>/dev/null` превратил бы это в пустой список — то есть непроведённая
      # проверка выглядела бы как проведённая и чистая. Тот же класс, что чинили
      # 26.08 в audit_public_repo.py.
      if book_out="$(kbengine fin accounts --from "$book" 2>&1)"; then
        from_book="$(printf '%s' "$book_out" | grep '→' || true)"
        if [ -n "$from_book" ]; then
          acc_pat="$(printf '%s\n%s' "$acc_pat" "$from_book" | grep -v '^$' | sort -u)"
        fi
      else
...
# ------------------------------------------- 7. настоящие имена людей
# Соседний гейт выше стережёт ДЕНЬГИ владельца и работал три недели, честно
# молча, пока в публичные фикстуры уезжали имя, должность и статус занятости
# живого человека. Утекли люди, а не суммы, — значит нужен свой детектор.
# Вынесен отдельным скриптом, потому что у него есть --self-test на настоящем
# git: краснеет на подсадке, молчит на чистом коммите и не молчит, когда
# источников имён нет вовсе.
if ! "$(dirname "$0")/no-people-names.sh"; then
  exit 1
fi
