...
describe('cover', () => {
  // Скриншот сюда НЕ попадает: как фон он обрезается по краям (cover) или
  // непредсказуемо оставляет поля (contain), и подпись поверх него наезжает на
  // содержимое снимка. Владелец это и увидел — у скриншота резалась шапка.
  // Поэтому картинка рисуется отдельным слоем и вписывается целиком, а cover
  // отвечает только за подложку под ней.
  it('картинка не подменяет подложку', () => {
