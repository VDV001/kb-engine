...
export interface Account {
  ...
  /**
   * Род счёта, прочитанный движком из имени: «Заморозка → Хранение» →
   * «Заморозка». Пустой у обычного счёта; отсутствует у сборки сервера
   * старше этих полей — тогда карточка разбирает имя сама.
   */
  group?: string
  /** Как счёт зовётся внутри рода: «Заморозка → Хранение» → «Хранение». */
