...
export interface Stats {
  ...
  /**
   * Записи, которые в файле каталога есть, а в числах выше — нет: прочитать их
   * не удалось. Приходят вместе со статистикой намеренно, потому что без них
   * «1455 записей» читается как вся база.
   */
  unreadable?: UnreadableEntry[]
  health: Health
}

/** Одна непрочитанная запись каталога. */
export interface UnreadableEntry {
  /** Место в файле. Единственный адрес, когда id прочитать тоже не удалось. */
  index: number
  id: number
  reason: string
}
...
export interface DocCard {
  ...
  from?: string
  to?: string
  /** Промежуточный участник: «Заказчик → Продакт → Лид» это два ребра. */
  via?: string
 ...
 *
 * `behind`, `no_anchors`, `unknown` и `stale_build` — четыре разных ответа:
 * «отстала», «сверять не с чем», «дату правки не знаем» и «отстала сборка, а не
 * страница». Сводить их значит показывать проверенным то, что не проверялось, —
 * или посылать человека править файл, который прав.
 */
export interface SourceState {
  ...
  no_anchors: boolean
  /**
   * Страница называет версию новее собранной: обновлять надо движок. Старая
   * сборка движка поля не отдаёт — тогда оно undefined, и ярлык остаётся
   * прежним, а не выдуманным.
   */
  stale_build?: boolean
  age_days: number
  facts: FreshnessFact[]
...
export interface Account {
  ...
  /**
   * Род счёта, прочитанный движком из имени: «Резерв → Наличные» →
   * «Резерв». Пустой у обычного счёта; отсутствует у сборки сервера
   * старше этих полей — тогда карточка разбирает имя сама.
   */
  group?: string
  /** Как счёт зовётся внутри рода: «Резерв → Наличные» → «Хранение». */
  name_in_group?: string
  /**
   * Код валюты счёта; ОТСУТСТВУЕТ у рублёвого. Пустота и есть «валюта книги»:
   * отдавай движок «RUB» каждому счёту, каждая страница обязана была бы
   * выучить новое поле, чтобы показать прежнее (#332).
   *
   * balance при этом — сумма в СВОЕЙ валюте, не пересчёт. Без кода рядом
   * «500.00» у долларового счёта читается как рубли.
   */
  currency?: string
  /** Курс, по которому счёт оценили, и сама оценка в валюте книги. */
  rate?: string
  base_value?: string
  /**
   * «Оценить нечем» — отдельным ответом. Ноль в base_value читался бы как
   * «денег нет»: ложь противоположного знака, деньги есть, курса нет.
   */
  unvalued?: boolean
}

/** Один курс, участвовавший в итоге рода, и день, на который он верен. */
export interface RateUsed {
  currency: string
  per_unit: string
  /** День подтверждения счёта: движок за курсом не ходит и не переоценивает. */
  on: string
}

/**
 * Итог по одному роду счетов, посчитанный движком.
 *
 * Витрина его не выводит сама. До #332 она складывала рода в TypeScript — и
 * эта вторая копия правила про деньги о валюте не знала: складывала доллары
 * с рублями и называла результат рублями, пока терминал считал верно.
 */
export interface AccountGroup {
  group: string
  total: string
  /** Счета рода, не вошедшие в total: оценить нечем. */
  unvalued?: string[]
  rates?: RateUsed[]
}
...
export interface Finances {
  ...
  accounts: Account[]
  /**
   * Сколько денег свободно — считает движок, а не витрина. Правило про то,
   * какие рода счетов входят в располагаемое, живёт в одном месте: посчитав
   * его здесь, витрина завела бы вторую копию, и та разошлась бы молча.
   * Старая сборка поля не отдаёт — тогда строки «свободно» просто нет.
   */
  free?: string
  /**
   * Итоги родов от движка. Старая сборка поля не отдаёт — тогда карточка
   * складывает сама, как делала раньше, и на рублёвой книге это верно.
   */
  groups?: AccountGroup[]
}
...
export interface ArchMap {
   ...
   *
   * Необязательное намеренно: сервер всегда отдаёт список, но старая сборка
   * движка про это поле не знает вовсе, и отличать «карта без примеров» от
   * «движок старше поля» странице незачем — лид в обоих случаях один.
   */
  examples?: string[]
  /**
   * Путь к собственному разбору проекта — странице с рисованными диаграммами и
   * тем, чего механическая карта сказать не умеет: что делалось по неделям,
   * зачем каждая технология, где ошиблись. Открывается маршрутом /kb/, то есть
   * только если каталог этот путь называет.
   *
   * Пусто — разбора нет, и раздел не показывается вовсе: пустая вкладка
   * читается как поломка, отсутствующая — как «здесь этого просто нет».
   */
  page?: string
  roots: ArchRoot[]
  roots_note?: string
...
export interface Dashboard {
  ...
  /** null — спросили и не смогли: отказ, а не отсутствие данных. */
...
export const api = {
   ...
   *
   * Пустой список у старых сборок приходит как null — нормализуем, витрина не
  ...
  async dashboard(): Promise<Dashboard> {
    const [stats, entries, audits, duplicates, analytics, analyticsConfig, finances, sources] =
      await Promise.all([
        ...
        // должны, поэтому отказ гасится здесь.
        //
        // Но гасится в null, а не в пустоту: «данных нет» и «данные не
        // читаются» — разные ответы, и подставляя пустоту, дашборд выдавал
        // отказ за законное состояние с ложной инструкцией «добавьте
        // --ledger» при уже переданном флаге.
        api.finances().catch(() => null),
