...
// runFinSync pairs the workbook with the ledger.
//
// Only --init for now: the two-way sync is the next piece of work, and a flag
...
func runFinSync(args []string, stdout, stderr io.Writer) int {
	...
	if !*initialize {
		fmt.Fprintln(stderr, "fin sync: two-way sync is not implemented yet — run with --init to pair the workbook with a ledger")
		return 2
	}
